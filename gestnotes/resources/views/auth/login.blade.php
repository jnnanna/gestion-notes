{{-- gestnotes/resources/views/auth/login.blade.php --}}
<!DOCTYPE html>
<html class="light" lang="fr">
<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width,initial-scale=1" />
    <title>Connexion — EduSecure</title>
...
<body class="bg-background-light dark:bg-background-dark text-[#0d121b] dark:text-white min-h-screen flex items-center justify-center p-6">
    <div class="w-full max-w-md">
        <header class="mb-6 text-center">
            ...
            <p class="text-sm text-[#4c669a] mt-2">Connexion au tableau de bord</p>
        </header>
        ...
        <main class="bg-white dark:bg-[#1a202c] rounded-xl border border-[#e7ebf3] dark:border-gray-700 p-6 shadow-sm">
            ...
            <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-4">
                ...
                <div class="relative">
                    <input type="password" name="password" placeholder="••••••••" required class="w-full h-12 rounded-lg border border-[#cfd7e7] dark:border-gray-600 bg-[#f8f9fc] dark:bg-gray-800 px-4 pr-12 focus:outline-none focus:border-primary" />
                </div>

                <div class="flex items-center justify-between text-sm">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="remember" class="w-4 h-4" /> <span>Se souvenir de moi</span>
                    </label>
                    <a href="{{ route('password.request') }}" class="text-primary font-medium">Mot de passe oublié?</a>
                </div>

                <button type="submit" class="mt-2 w-full h-12 rounded-lg bg-primary text-white font-bold hover:bg-blue-700 transition-colors">Se connecter</button>

                <div class="pt-3 text-center text-sm text-[#4c669a]">
                    Pas de compte ? <a href="{{ route('register') }}" class="text-primary font-medium">S'inscrire</a>
                </div>
            </form>
        </main>
